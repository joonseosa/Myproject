  ...
  <div class ="tb_box">
	...
	<td class="tb2">
		<table>
	  	  <thead>
			<tr>
			<p style="font-size:24px; font-family:sans-serif; font-weight:bold;">Admin Page- 주문관리 </p>
			</tr>
			<tr> 
...
<!-- 상세보기기능 주석처리  -->
		...
		<table>
					<h1 style="text-align: center">
						<a href="/OrderMgr?presentPage=1" style="color: black;font-size:0.7em;"> [start] </a>
						<c:if test="${pdto.presentPage!=1}">
							<a href="/OrderMgr?presentPage=${pdto.presentPage-1}" style="color: black;font-size:0.7em;"> < 
						</c:if>
						</a>
						<c:forEach var="i" begin="${pdto.startPage}" end="${pdto.endPage}">
							<c:if test="${pdto.presentPage==i}">
								<a href="/OrderMgr?presentPage=${i}"><span
									style="color: red;font-size:0.7em;"><c:out value="[${i}]" /></span></a>
							</c:if>
							<c:if test="${pdto.presentPage!=i}">
								<a href="/OrderMgr?presentPage=${i}" style="color: black;font-size:0.7em;"><c:out value="[${i}]" /></a>
							</c:if>
						</c:forEach>
						<c:if test="${pdto.presentPage!=pdto.endPage}">
							<a href="/OrderMgr?presentPage=${pdto.presentPage+1}" style="color: black;font-size:0.7em;"> > 
						</c:if>
						</a> <a href="/OrderMgr?presentPage=${pdto.allPage}" style="color: black;font-size:0.7em;"> [end] </a>
					</h1>
				</table>

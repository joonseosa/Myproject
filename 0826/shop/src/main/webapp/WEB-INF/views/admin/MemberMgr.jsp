...
<%-- 				<td class="tbodystyle"><a href="javascript:Update('${mdto.mem_id}')">수정하기</a></td> --%>
	...
	<table>
					<h1 style="text-align: center">
						<a href="/MemberMgr?presentPage=1" style="color: black"> [start] </a>
						<c:if test="${pdto.presentPage!=1}">
							<a href="/MemberMgr?presentPage=${pdto.presentPage-1}" style="color: black"> < 
						</c:if>
						</a>
						<c:forEach var="i" begin="${pdto.startPage}" end="${pdto.endPage}">
							...
							<c:if test="${pdto.presentPage!=i}">
								<a href="/MemberMgr?presentPage=${i}" style="color: black"><c:out value="[${i}]" /></a>
							</c:if>
						</c:forEach>
						<c:if test="${pdto.presentPage!=pdto.endPage}">
							<a href="/MemberMgr?presentPage=${pdto.presentPage+1}" style="color: black"> > 
						</c:if>
						</a> <a href="/MemberMgr?presentPage=${pdto.allPage}" style="color: black"> [end] </a>
					</h1>
				</table>

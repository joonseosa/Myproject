...
<%--   <c:when test="${idKey!=null}"> --%>
	...
	<table>
					<h1 style="text-align: center">
						<a href="/ProductMgr?presentPage=1" style="color: black"> [start] </a>
						<c:if test="${pdto.presentPage!=1}">
							<a href="/ProductMgr?presentPage=${pdto.presentPage-1}" style="color: black"> < 
						</c:if>
						</a>
						<c:forEach var="i" begin="${pdto.startPage}" end="${pdto.endPage}">
							...
							<c:if test="${pdto.presentPage!=i}">
								<a href="/ProductMgr?presentPage=${i}" style="color: black"><c:out value="[${i}]" /></a>
							</c:if>
						</c:forEach>
						<c:if test="${pdto.presentPage!=pdto.endPage}">
							<a href="/ProductMgr?presentPage=${pdto.presentPage+1}" style="color: black"> > 
						</c:if>
						</a> <a href="/ProductMgr?presentPage=${pdto.allPage}" style="color: black"> [end] </a>
					</h1>
				</table>
